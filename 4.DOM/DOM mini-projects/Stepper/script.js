...
function nextEvent(eventObj) {
    ...
    if ( currentStep > 4 ){
        // Chnaging Message Content
        const mesgDiv = document.querySelector(".message")
        mesgDiv.innerText = pageMessageObject[5]
        return 
    }
    ...
    const currStepNumberDiv = document.querySelectorAll(`.stepper .step-number`)[currentStep - 1]
    currStepNumberDiv.querySelector("div").innerText = "O"
    ...
    if( currentStep < 4 ){
        ...
        const nextStepNumberDiv = document.querySelectorAll(`.stepper .step-number`)[currentStep]
